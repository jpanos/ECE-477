...
def getShellcommand():
	...
	#gsstreamerPL = 'nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)1920, height=(int)1080, format=(string)NV12, framerate=(fraction)30/1 ! nvvidconv flip-method=2 ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink'
	gsstreamerPL = 'nvarguscamerasrc sensor_id=0 ! nvoverlaysink'
	return gsstreamerPL
